...
-- * Generation date: Mon Dec 12 09:25:52 2022 
...
create table t_collaborator_job (
     ...
     constraint ID_is_ID primary key (idJob, idPerson));
...
create table t_job (
     ...
     name varchar(50) not null,
     constraint ID_t_job_ID primary key (idJob));
...
create table t_scope (
     idScope bigint not null auto_increment,
     name varchar(50) not null,
     constraint ID_t_scope_ID primary key (idScope));
...
create table t_specification (
     idSpecification bigint not null auto_increment,
     myKey varchar(100) not null,
     myValue varchar(200) not null,
     idEventType bigint,
     constraint ID_t_specification_ID primary key (idSpecification));
...
alter table t_collaborator_job add constraint FKis_t_j
     foreign key (idJob)
...
alter table t_event add constraint FKhave_3_FK
...
alter table t_eventType add constraint FKbind_FK
     foreign key (idScope)
     references t_scope (idScope);
...
alter table t_specification add constraint FKhave_2_FK
     foreign key (idEventType)
     references t_eventType (idEventType);
...
create unique index ID_is_IND
     on t_collaborator_job (idJob, idPerson);
...
create index FKhave_3_IND
     on t_event (idStudentGroup);
...
create index FKbind_IND
     on t_eventType (idScope);

create unique index ID_t_job_IND
     on t_job (idJob);
...
create unique index ID_t_scope_IND
     on t_scope (idScope);

create unique index ID_t_section_IND
     on t_section (idSection);

create unique index ID_t_specification_IND
     on t_specification (idSpecification);

create index FKhave_2_IND
     on t_specification (idEventType);
